...
import { Home, LayoutDashboard, Users, HeartPulse, History, Database, Stethoscope } from 'lucide-react';
      ...
      <div className="flex justify-around items-center h-16 px-2">
        ...
        {userRole !== 'Patient' && (userRole !== 'Supervisor' || hasPermission('view_dashboard')) && (
          <button onClick={() => onNavigate('dashboard')} className={navItemClass(currentView === 'dashboard')}>
            <div className={`p-1.5 rounded-full ${currentView === 'dashboard' ? 'bg-teal-100' : ''}`}>
        ...
        {/* History (Center Action for Patients) */}
        ...
        {(userRole === 'Admin' || userRole === 'Doctor' || hasPermission('manage_users')) && (
          <button onClick={() => onNavigate('users')} className={navItemClass(currentView === 'users')}>
            <Users size={22} strokeWidth={currentView === 'users' ? 2.5 : 2} />
            <span>{userRole === 'Doctor' ? 'المرضى' : 'المستخدمين'}</span>
          </button>
        )}

        {/* Specialties (If space permits or user is not admin) */}
        {!(userRole === 'Admin' || userRole === 'Doctor') && (
            <button onClick={() => onNavigate('specialties')} className={navItemClass(currentView === 'specialties')}>
                <Stethoscope size={22} strokeWidth={currentView === 'specialties' ? 2.5 : 2} />
                <span>التخصصات</span>
            </button>
        )}
